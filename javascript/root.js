...
(async () => {
    ...
    const MediaPlayer = {
        ...
        PreviousButton: document.querySelector('button#PreviousButton'),
        /** @type {HTMLButtonElement} */
    ...
    [].forEach.call(Array.from(document.querySelectorAll('.sidebar_item')), function (elem, index) {
        elem.addEventListener('click', async function () {
            ...
            await FetchMusic(MetaData, index, ({ Player, Title, Artist, Album, Released, Image, _src }, error) => {
                ...
                NewMetaData({ Title, Artist, Album, Image }, MediaPlayer.PreviousButton, MediaPlayer.NextButton);
                _isPlaying && Elements.MusicPlayer.play().then(_ => { }).catch(() => _isPlaying = false);
            });
        });
    });
